...
import edu.sabanciuniv.berry.repository.FollowerRepository;
...
import edu.sabanciuniv.berry.service.FollowerService;
...
public class HomeController {
	...
	public String profile(Model md) {		
		...
		//md.addAttribute("followerList", followerRepository.findByUserNameContaining(currentUser.getFollowers());
    ...
    public boolean addFollower(@RequestParam(value="username") Model model,String username)
    {
		boolean flag = false;
  
	    Follower follower = (Follower)userRepository.findByUserNameContaining(username);
	    try {
	        Optional<User> followee = userRepository.findById(username);
	        if (followee.get().getFollowers().contains(follower)) {
	            followee.get().getFollowers().remove(follower);
	            System.out.println("removed");
	            flag=true;

	        } else {
	            followee.get().getFollowers().add(follower);
